...
RADIO_BUTTONS = (
    {"text": "Set Zone Start", "value": 0, "command": lambda f: lambda: f.setfocus(0)},
...
class ElasticIntervalFrame(AbstractTabFrame):
    ...
    def __init__(self, parent, handler, next_frame):
        ...
        self.interval_lines = [
            self.canvas.plot("interval{}".format(i), [], [], **LINE_STYLE)
            for i in (1, 2)
        ]
        ...
        self.canvas.set_labels(
            "Load vs. Displacement", "Displacement (in.)", "Load (lbs.)"
        )
        self.canvas.draw()
        self.sample = None
        self.build()
    ...
    def content_update(self):
        ...
        self.canvas.set_plotrange(
            (disp[0], 1.3 * np.max(disp)), (0, 1.3 * np.max(load))
        )
    ...
    def update_lines(self):
        ...
        self.interval_lines[0].set_data(
            self.canvas.plotrange[0], [e_load[0], e_load[0]]
        )
        self.interval_lines[1].set_data(
            self.canvas.plotrange[0], [e_load[-1], e_load[-1]]
        )
    ...
    def build(self):
        ...
        Button(
            self.controlframe,
            font=GUI_FONT,
            text="Auto Elastic Zone",
            command=self.autoelastic_click,
        ).pack(side=LEFT, padx=10, pady=10)
        Button(
            self.controlframe, font=GUI_FONT, text="Done", command=self.on_next
