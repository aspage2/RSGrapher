...
class UTSFrame(SamplePlotFrame):
    ...
    def __init__(self, parent):
        ...
        self.uts_text = self.canvas.axes.text(
            0, 0, "", bbox=BBOX, va="bottom", ha="left"
        )
        self.yield_text = self.canvas.axes.text(
            0, 0, "", bbox=BBOX, va="top", ha="left"
        )
        self.yieldline = self.canvas.plot("yieldline", [], [], **ELASTIC_STYLE)
        self.yieldline_data = self.canvas.plot(
            "yieldline_data", [], [], **HIGHLIGHT_STYLE
        )
    ...
    def set_sample(self, sample):
        ...
        self.canvas.set_data(
            strain[sample.zero : sample.cutoff], stress[sample.zero : sample.cutoff]
        )

        # Ultimate Tensile Strength
        self.uts.set_data(strain[sample.peak_load], stress[sample.peak_load])

        s = "UTS: {0:.{1}f} ksi".format(
            stress[sample.peak_load] / 1000.0, 3 - sample.precision
        )
        ...
        s = "Yield Strength: {0:.{1}f} ksi".format(
            stress[yield_ind] / 1000.0, 3 - sample.precision
        )
