...
class PeakLoadFrame(SamplePlotFrame):
    ...
    def __init__(self, parent):
        super().__init__(
            parent, "Load vs. Displacement", annotation_id="peakload_annotation"
        )
        self.peakpoint = self.canvas.plot("peakloadpoint", [], [], **POINT_STYLE)
        self.peaktext = self.canvas.axes.text(
    ...
    def set_sample(self, sample):
        ...
        self.canvas.set_data(
            disp[sample.zero : sample.cutoff], load[sample.zero : sample.cutoff]
        )
        ...
        s = "Peak Load: {0:.{1}f} kips".format(
            load[sample.peak_load] / 1000.0, 3 - sample.precision
        )
