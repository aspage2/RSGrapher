...
class AppWindow(Tk):
    ...
    def bind_menu_actions(self):
        ...
        filemenu.add_command(
            label="New Project", command=self._project_handler.new_project
        )
        filemenu.add_command(
            label="Open Project", command=self._project_handler.open_project
        )
        filemenu.add_separator()
        filemenu.add_command(
            label="Project Info", command=self._project_handler.edit_info
        )
        filemenu.add_separator()
        ...
        samplemenu.add_command(
            label="Add Sample", command=self._project_handler.new_sample
        )
        samplemenu.add_command(
            label="Select Sample", command=self._project_handler.select_sample
        )
        samplemenu.add_command(
            label="Delete This Sample", command=self._project_handler.delete_curr_sample
        )
        menubar.add_cascade(label="Sample", menu=samplemenu)

        projectmenu = Menu(menubar, tearoff=False)
        projectmenu.add_command(
            label="Set Project Date", command=self._project_handler.set_date
        )
